...
def makeValenceModel():
    valence_model = Sequential()
    data_len = DATA_LEN
    cnt = step
    while cnt<data_len and data_len - cnt >= resource_barrier:
        valence_model.add(Conv1D(data_len-cnt,kernel_size=cnt,activation = 'relu'))
...
def makeArousalModel():
    arousal_model = Sequential()
    data_len, cnt=DATA_LEN, step

    while cnt<data_len and data_len - cnt >= resource_barrier:
        arousal_model.add(Conv1D(data_len-cnt,kernel_size=cnt,activation = 'relu'))
        data_len = int(data_len*(1-dropout_rate))
        cnt += step
...
trainAndTest('arousal.txt',arousal_model)
trainAndTest('valence.txt',arousal_model)